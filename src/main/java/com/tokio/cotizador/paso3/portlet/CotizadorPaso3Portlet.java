...
import com.tokio.cotizadorModular.Bean.CaratulaBajaDatosCaratula;
import com.tokio.cotizadorModular.Bean.CaratulaBajaDatosGeneral;
import com.tokio.cotizadorModular.Bean.CaratulaBajaUbicaciones;
import com.tokio.cotizadorModular.Bean.CaratulaResponse;
import com.tokio.cotizadorModular.Bean.CotizadorDataResponse;
import com.tokio.cotizadorModular.Bean.InfoAuxPaso1;
...
@Component(immediate = true, property = { "com.liferay.portlet.display-category=category.sample",
		...
		"javax.portlet.display-name=CotizadorPaso3Portlet Portlet",
...
public class CotizadorPaso3Portlet extends MVCPortlet {
	...
	public void render(RenderRequest renderRequest, RenderResponse renderResponse)
		...
		if (!isBajaEndoso()) {
			if(Validator.isNotNull(infoPaso1.getSubEstado())){
				if(!infoPaso1.getSubEstado().equals( CotizadorPaso3PortletKeys.COTIZADO_SUSCRIPTOR)){		
	...
	public int botonesCaratula(String subEstado, int idPerfil) {
		if (Validator.isNotNull(subEstado)) {
			if ((idPerfil < 4) && ((subEstado.trim()
					.equals(CotizadorPaso3PortletKeys.COTIZADO_SUSCRIPTOR))
					|| (subEstado.trim().equals(CotizadorPaso3PortletKeys.RECHAZO_VBA_482)))) {
				return 1;
			}
			if ((idPerfil > 3) && ((subEstado.trim().equals(CotizadorPaso3PortletKeys.EDO_SUBGIRO))
					|| (subEstado.trim().equals(CotizadorPaso3PortletKeys.EXEDE_LIMITES))
					|| (subEstado.trim().equals(CotizadorPaso3PortletKeys.REVIRE_SUSCRIPTOR)))) {
				return 2;
			}
		}
		return 0;
...
//			auxRenovacion();
			...
			switch (infCotizacion.getTipoCotizacion()) {
				case FAMILIAR:
					...
					nombreCotizador = CotizadorPaso3PortletKeys.TITULO_FAMILIAR;
					break;
				case EMPRESARIAL:
					infCotizacion.setPantalla("moduloEmpresarial");
					nombreCotizador = CotizadorPaso3PortletKeys.TITULO_EMPRESARIAL;
					break;
				default:
